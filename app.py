import streamlit as st
import random

def calculate_light_score(stress, focus, calm, poem):
    # スコア計算を修正
    # ストレスを逆転させて、3つの平均を取る方式に変更
    reversed_stress = 5 - stress  # ストレスを反転（高いほど良い状態に）
    raw_score = (reversed_stress + focus + calm) / 3
    score = round(max(1, min(5, raw_score)), 2)
    
    # アドバイス生成
    if score >= 4.0:
        advice = "素晴らしい！今日のあなたは光り輝いています。そのままのあなたで進んでください。"
        celebration = True
    elif score >= 2.5:
        advice = "まずまずの調子です。少しだけ自分のための時間を作ってみましょう。"
        celebration = False
    else:
        # 低スコア時のおまじない（ランダム）
        advices = [
            "【おまじない】深呼吸してリラックス。ゆっくり息を吐いてみて。",
            "【おまじない】自分の好きな食べ物や音楽など、好きなものを何でもいいから身につけて（取り入れて）みて。",
            "【おまじない】魔法の言葉だよ。「大丈夫」。声に出してみて。",
            "【おまじない】ふぅーっと一息ついて、空を見上げてごらん。"
        ]
        advice = random.choice(advices)
        celebration = False
    
    # 詩が入力されていたら追加メッセージ
    if len(poem.strip()) > 0:
        advice += f"\n\n✨ 詩の中に光が宿っています。「{poem[:15]}…」を胸の中心で味わってください。"
    
    return score, advice, celebration

# アプリUI
st.set_page_config(page_title="光スコア診断アプリ", page_icon="✨", layout="centered")
st.title("✨ 光スコア診断アプリ")
st.write('ストレス・集中・静けさ・詩から、今日の"光の度合い"を算出します')

st.subheader("■ 今日の状態を入力してください")
stress = st.slider("ストレス（0〜5）", 0, 5, 2)
focus = st.slider("集中（0〜5）", 0, 5, 3)
calm = st.slider("静けさ（0〜5）", 0, 5, 3)
poem = st.text_area("今日の詩、または感じた言葉（任意）", "")

if st.button("✨ 光スコアを計算する"):
    score, advice, celebration = calculate_light_score(stress, focus, calm, poem)
    
    if celebration:
        st.balloons()
    
    st.subheader("■ 診断結果")
    st.write(f"今日の光スコア： **{score} / 5.0**")
    
    st.info(f"💡 今日のあなたへのメッセージ：\n\n {advice}")
    
    st.write("---")
    st.caption("Powered by Singularity Education")





